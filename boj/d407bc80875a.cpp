...
int main() {
	...
	for (int i = 0; i < n - 1; ++i) {
		for (int j = 0; j < m - 1; ++j) {
			...
			else if (cnt == 0b1110) match[i][j][num(flag[0][0])]++;
			else if (cnt == 0b1101) match[i][j + 1][num(flag[0][1])]++;
			else if (cnt == 0b1011) match[i + 1][j][num(flag[1][0])]++;
			else if (cnt == 0b111) match[i + 1][j + 1][num(flag[1][1])]++;
		}
	}
